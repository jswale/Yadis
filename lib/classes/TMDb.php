...
class TMDb
{
	 ...
	 * @return void
	 */
	public function __construct($apikey, $default_lang = 'en', $config = FALSE, $scheme = self::API_SCHEME)
	{
		$this->_apikey = (string) $apikey;
		$this->_apischeme = ($scheme == self::API_SCHEME) ? self::API_SCHEME : self::API_SCHEME_SSL;
	...
	public function addFavoriteMovie($account_id, $session_id = NULL, $movie_id = 0, $favorite = TRUE)
	{
		...
		return $this->_makeCall('account/'.$account_id.'/favorite', $params, $session_id, self::POST);
	}
	...
	public function addMovieToWatchlist($account_id, $session_id = NULL, $movie_id = 0, $watchlist = TRUE)
	{
		...
		return $this->_makeCall('account/'.$account_id.'/movie_watchlist', $params, $session_id, self::POST);
	}
	...
	public function addMovieRating($session_id = NULL, $movie_id = 0, $value = 0)
	{
		...
		return $this->_makeCall('movie/'.$movie_id.'/rating', $params, $session_id, self::POST);
	}
	 ...
	 * @param const $imagetype			Image type: self::IMAGE_BACKDROP, self::IMAGE_POSTER, self::IMAGE_PROFILE
	...
	{
		$headers = $this->_makeCall($uri, NULL, NULL, self::HEAD);
	 ...
	 * @param const $method				self::GET or TMDb:POST (default self::GET)
	 * @return TMDb result array
	 */
	private function _makeCall($function, $params = NULL, $session_id = NULL, $method = self::GET)
	{
		...
		$url = $this->_apischeme.self::API_URL.'/'.self::API_VERSION.'/'.$function.'?'.http_build_query($auth_array, '', '&');
